<?php
	...
	include_once("Animal_class.php"); //entidade
	...
	//operações básicas de banco de dados
		...
		public function cadastrar($cont){
			try{
				...
				"INSERT INTO animais_procurados(nome, idade, raca, cor, porte, descricao, foto)
		...
		public function alterar($cont){
			try{
				$stmt = $this->con->prepare(
				"UPDATE animais_procurados SET nome=:nome, 
		...
		public function excluir($cont){
			try{
				$num = $this->con->exec("DELETE FROM animais_procurados WHERE id = " . $cont->getId());
		...
		public function listar($query = null){		
			...
			try{
				if($query == null){
					$dados = $this->con->query("SELECT * FROM animais_procurados");
		...
		public function exibir($id){			
			try{				
				$lista = $this->con->query("SELECT * FROM animais_procurados WHERE id = " . $id);
